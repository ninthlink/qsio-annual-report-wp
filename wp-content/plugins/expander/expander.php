...
function wpex_main($atts, $content = null) {
	...
	$new_string = '<a onclick="wpex_toggle(' . $rnum . ', \'' . addslashes($more) . '\', \'' . addslashes($less) . '\'); return false;" class="wpex-link wpexlinkmore" id="wpexlink' . $rnum . '" href="#">' . addslashes($more) . '</a>' . "\n";
...
function wpex_javascript() {
	...
	function wpex_toggle(id, more, less) {
		...
		if(el.innerHTML == more){
			el.innerHTML = less;
			el.className = el.className.replace("wpexlinkmore","wpexlinkless");}
		else{
			el.innerHTML = more;
			el.className = el.className.replace("wpexlinkless","wpexlinkmore");}
